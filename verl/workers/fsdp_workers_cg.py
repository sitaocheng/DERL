...
class RewardNode(nn.Module):
    def __init__(self):
        ...
        # 可学习的参数
        self.weights = nn.Parameter(torch.rand(len(self.operations)))
    ...
    def forward(self, x, y):
        ...
        op_name = self.operations[op_idx.item()]
        op_func = self.op_map[op_name]
        
        output = op_func(x, y)
        log_prob = m.log_prob(op_idx)
        
        return output, log_prob, op_name

    def evaluate_action(self, x, y, op_name):
        """评估一个给定动作的对数概率和熵"""
        probs = torch.softmax(self.weights, dim=-1)
        m = Categorical(probs)
        
        op_idx = torch.tensor(self.operations.index(op_name), device=self.weights.device)
        log_prob = m.log_prob(op_idx)
        entropy = m.entropy() # 熵，用于鼓励探索
        
        op_func = self.op_map[op_name]
        output = op_func(x, y)
        
        return output, log_prob, entropy
...
class RewardModel(nn.Module):
    def __init__(self):
        ...
        self.node2_1 = RewardNode()

    def forward(self, g1_val, g2_val, g3_val, g4_val):
        """前向传播，用于采样和生成数据"""
        # 确保输入是Tensor
        g1 = torch.tensor(g1_val, dtype=torch.float32, device=self._get_device())
        g2 = torch.tensor(g2_val, dtype=torch.float32, device=self._get_device())
        g3 = torch.tensor(g3_val, dtype=torch.float32, device=self._get_device())
        g4 = torch.tensor(g4_val, dtype=torch.float32, device=self._get_device())
        ...
        # 返回最终奖励，所有动作的总对数概率，以及具体采取的动作
        return final_reward_logit, torch.stack(log_probs).sum(), sampled_ops

    def evaluate_actions(self, g1_val, g2_val, g3_val, g4_val, actions):
        """给定一批状态和动作，重新评估其对数概率和熵"""
        g1 = torch.tensor(g1_val, dtype=torch.float32, device=self._get_device())
        g2 = torch.tensor(g2_val, dtype=torch.float32, device=self._get_device())
        g3 = torch.tensor(g3_val, dtype=torch.float32, device=self._get_device())
        g4 = torch.tensor(g4_val, dtype=torch.float32, device=self._get_device())
        ...
        node1_1_output, log_prob_1_1, entropy_1_1 = self.node1_1.evaluate_action(g1, g2, actions['node1_1'])
        node1_2_output, log_prob_1_2, entropy_1_2 = self.node1_2.evaluate_action(g3, g4, actions['node1_2'])
        
        log_probs.extend([log_prob_1_1, log_prob_1_2])
        entropies.extend([entropy_1_1, entropy_1_2])

        _, log_prob_2_1, entropy_2_1 = self.node2_1.evaluate_action(node1_1_output, node1_2_output, actions['node2_1'])
        
        log_probs.append(log_prob_2_1)
        entropies.append(entropy_2_1)

        return torch.stack(log_probs).sum(), torch.stack(entropies).sum()

    def _get_device(self):
        return self.node1_1.weights.device


# 定义一个简单的数据结构来传递数据，模仿 DataProto
class TrainingData:
    def __init__(self):
        self.data = defaultdict(list)
    
    def push(self, state, actions, old_log_prob, reward):
        self.data['states'].append(state)
        self.data['actions'].append(actions)
        self.data['old_log_probs'].append(old_log_prob)
        self.data['rewards'].append(reward)

    def to_tensors(self, device):
        # 这个函数需要根据您的具体状态格式进行调整
        # 这里我们假设状态是 (g1, g2, g3, g4)
        g1 = torch.tensor([s[0] for s in self.data['states']], device=device)
        g2 = torch.tensor([s[1] for s in self.data['states']], device=device)
        g3 = torch.tensor([s[2] for s in self.data['states']], device=device)
        g4 = torch.tensor([s[3] for s in self.data['states']], device=device)
        
        # 将动作列表转换为适合批处理的格式
        actions_batched = {
            'node1_1': [a['node1_1'] for a in self.data['actions']],
            'node1_2': [a['node1_2'] for a in self.data['actions']],
            'node2_1': [a['node2_1'] for a in self.data['actions']],
        }

        return {
            "g1": g1, "g2": g2, "g3": g3, "g4": g4,
            "actions": actions_batched,
            "old_log_probs": torch.stack(self.data['old_log_probs']).detach(),
            "rewards": torch.tensor(self.data['rewards'], device=device),
        }


class RewardModelWorker:
    """
    一个专门用于训练 RewardModel 的 Worker 类。
    """
    def __init__(self, config):
        self.config = config
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Worker is using device: {self.device}")
        
        # 模型和优化器将在 init_model 中初始化
        self.model = None
        self.optimizer = None
    ...
    def generate_samples(self, batch_of_states):
        """
        根据一批输入状态，使用当前模型生成训练样本。
        这取代了原Worker中的 `generate_sequences`。
        """
        print(f"\nGenerating {len(batch_of_states)} samples...")
        ...
        with torch.no_grad():
            for state in batch_of_states:
                ...
                # 使用模型进行前向传播（采样）
                reward, old_log_prob, actions = self.model(g1, g2, g3, g4)
                
                # 保存样本
                training_data.push(state, actions, old_log_prob, reward)
        
        print("Sample generation complete.")
        return training_data

    def update_model(self, training_data: TrainingData):
        """
        使用收集到的样本数据，更新模型参数。
        这取代了原Worker中的 `update_actor`。
        """
        if not training_data.data['states']:
        ...
        # 将数据转换为批处理的Tensors
        batch = training_data.to_tensors(self.device)
        
        print(f"\nUpdating model for {ppo_epochs} epochs...")
        
        for epoch in range(ppo_epochs):
            # 重新评估旧动作在新策略下的对数概率和熵
            # 我们需要逐一样本进行，因为 evaluate_actions 目前不是批处理的
            new_log_probs_list = []
            entropies_list = []
            for i in range(len(batch['g1'])):
                actions_i = {key: val[i] for key, val in batch['actions'].items()}
                log_prob, entropy = self.model.evaluate_actions(
                    batch['g1'][i], batch['g2'][i], batch['g3'][i], batch['g4'][i], actions_i
                )
            ...
            # 2. 计算优势函数 (Advantage)
            # 在这个场景下，我们没有价值函数，直接使用奖励作为优势的代理
            advantages = batch['rewards']
            
            # 3. 计算策略损失（Clipped Surrogate Objective）
            ...
            print(f"  Epoch {epoch+1}/{ppo_epochs} | Total Loss: {loss.item():.4f} | Policy Loss: {policy_loss.item():.4f} | Entropy: {-entropy_loss.item():.4f}")
