...
def compute_avg_score(reward_list, math_output_file, gsm8k_output_file, loop_num, rollout_num):
    """
    ...
    print(f"GSM8K平均得分: {gsm8k_score:.4f}")
    ...
    with open(f"./outputs/meta_grpo_dlc/checked_rollouts/loop_{loop_num}_checked_with_reward.json", 'w', encoding='utf-8') as f:
        json.dump(reward_list, f, ensure_ascii=False, indent=4)
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            result = self.val_reward_fn(test_batch, return_dict=True) # edited by DERL
    ...
    def fit_outer(self):
        ...
        rollout_reward_file = f"./outputs/meta_grpo_dlc/checked_rollouts/loop_{self.config.outer_iter}_checked_with_reward.json"
        
        with open(rollout_reward_file, "r") as f:
            rollout_rewards = json.load(f)
        
        rollouts = [i['rollout'] for i in rollout_rewards if i['reward'] != -1.0]
        rewards = [i['reward'] for i in rollout_rewards if i['reward'] != -1.0]

        while len(rollouts) != 8:
            rollouts.append(rollouts[0])
            rewards.append(rewards[0])

        print("reward without -1")

        rewards = [0.0 if i == -1.0 else i for i in rewards]
        
        print("number of rollouts, ", len(rollouts))
        
        # 确保rollout数量和reward数量一致
        assert len(rollouts) == len(rewards), "Rollouts and rewards must have the same length"

        # MODIFIED: 简化训练循环，只执行一个step
        # 我们将总训练步数强制设为1
        self.total_training_steps = 1
        progress_bar = tqdm(total=self.total_training_steps, desc="Single Step Training")
        
        self.global_steps = 1  # 开始第一个也是唯一一个step
        metrics = {}
        timing_raw = {}
        ...
        with _timer("step", timing_raw):
            ...
            # 离线数据处理路径
            # ==========================================================
            with _timer("offline_data_prep", timing_raw):
                # 1. Tokenize prompts 和 rollouts
                ...
                # 2. 组合成完整的输入
                batch_size = len(rollouts) # batch_size 现在是 4
                input_ids = torch.cat([prompt_ids.repeat(batch_size, 1), response_ids], dim=1)
                prompt_attention_mask = torch.ones_like(prompt_ids.repeat(batch_size, 1))
                attention_mask = torch.cat([prompt_attention_mask, response_attention_mask], dim=1)
                position_ids = torch.arange(0, prompt_len + response_len, device=input_ids.device).unsqueeze(0).repeat(batch_size, 1)

                # 3. 构建 DataProto 对象
                offline_batch_dict = {
                    "input_ids": input_ids, "attention_mask": attention_mask, "position_ids": position_ids,
                    "prompts": prompt_ids.repeat(batch_size, 1), "responses": response_ids,
                }
                batch = DataProto(batch=TensorDict(offline_batch_dict, batch_size=[batch_size]))

            # 4. 注入预先计算好的奖励 (Reward)
            with _timer("reward", timing_raw):
                token_level_scores = torch.zeros_like(response_ids, dtype=torch.float32)
                response_lengths = response_attention_mask.sum(dim=1)
                ...
                token_level_scores[torch.arange(batch_size)[valid_indices], last_token_indices[valid_indices]] = reward_values[valid_indices]

                print("Token level scores:", token_level_scores)
                print("Response lengths:", response_lengths)
                print("Last token indices:", last_token_indices)
                print("Reward values:", reward_values)
            ...
            with _timer("adv", timing_raw):
                ...
                batch = compute_advantage(
                    ...
                    # MODIFIED: num_repeat 设为4，因为这4条数据都来自同一个prompt
                    num_repeat=len(rollouts),
        ...
        print("\nSingle step training finished with hardcoded data.")
        pprint(f"Final metrics: {metrics}")
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if current_time - start_time >= 60*60*2:
                    print("exceed max training time!!! three and a half hours!!!")
                    is_last_step = True
